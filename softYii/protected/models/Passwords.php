...
class Passwords extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('location, username, password, DateCreated, LastUpdate, id_user', 'required'),
			array('id_user', 'numerical', 'integerOnly'=>true),
			array('location, username, password', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, location, username, password, DateCreated, LastUpdate, id_user', 'safe', 'on'=>'search'),
		);
	}
